...
func Post_api_oauth_authorizeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_api_oauth_authorizeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_oauth_authorize",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_oauth_authorizeHandler(cfg),
	}
}
